...
--
-- Name: delete_shop_review(integer, character varying, timestamp without time zone, text); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.delete_shop_review(shop_id_ integer, user_login_ character varying, review_date_ timestamp without time zone, review_text_ text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
        DECLARE
...
ALTER FUNCTION public.delete_shop_review(shop_id_ integer, user_login_ character varying, review_date_ timestamp without time zone, review_text_ text) OWNER TO postgres;
...
CREATE FUNCTION public.get_client_orders(login character varying) RETURNS TABLE(book_title_ character varying, genre_ character varying, quantity_ integer, sum_to_pay_ numeric, order_status_ character varying, date_of_order_ timestamp without time zone, date_of_return_ timestamp without time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
        BEGIN
            RETURN QUERY
                SELECT
                    title,genre_type, quantity, sum_to_pay, order_status, date_of_order, date_of_return
                FROM
...
--
-- Name: seconds_to_dhms(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.seconds_to_dhms(seconds integer) RETURNS character varying
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
        DECLARE
            MINUTE integer = 60;
            HOUR integer = 60 * 60;
            DAY integer = 60 * 60 * 24;

        BEGIN
            RETURN
                (seconds / DAY)::varchar || ' Days ' ||
                ((seconds % DAY) / HOUR) :: varchar || ' Hours ' ||
                (((seconds % DAY) % HOUR) / 60) :: varchar || ' Minutes ' ||
                (seconds % MINUTE):: varchar || ' Seconds ';
    END
    $$;


ALTER FUNCTION public.seconds_to_dhms(seconds integer) OWNER TO postgres;
...
CREATE VIEW public.sales AS
 SELECT edition.concrete_shop,
   ...
   FROM public.client_order,
    ...
    public.edition,
    public.book
  WHERE (((client_order.order_status)::text <> ALL ((ARRAY['Отменён'::character varying, 'В коризне'::character varying])::text[])) AND (client_order.order_id = chosen.order_id) AND (chosen.edition_number = edition.edition_number) AND (edition.authority_id = authority.authority_id) AND (authority.edition_book = book.book_id));
...
1	BigBook shop # 1	5	79007	Украина	Львов	ул. Андрея Головко, 1	9
2	BigBook shop # 2	4	65125	Украина	Одесса	ул. Мельницкая, 13	13
3	BigBook shop # 3	4	65125	Украина	Одесса	ул. Довженко, 3	17
4	BigBook shop # 4	4	4050	Украина	Киев	ул. Крещатик	21
5	BigBook shop # 5	4	4050	Украина	Киев	ул. Владимирская	25
\.
...
39	Канаева	Тамара	+380255678564	tamara_kanvaeva@gmail.com	TamaraKanaeva605	161450, г. Долгоруково, ул. Крутицкий Вал, дом 181, квартира 540
95	Александр	Петров	+380999999999	asd@gmail.com	test_login	test_address
106	dummy	client	+380000000000	dummy_client@gmail.com	dummy_client	-
\.
...
146	15	39	161450, г. Долгоруково, ул. Крутицкий Вал, дом 181, квартира 540	Доставлен	450.00	Карта	3	\N	nice price	2022-08-21 15:25:07	2022-08-21 15:26:31
...
14	12	2	1	750.00	2013-02-13	35	331	Твёрдый	Офсетная
...
8	Семёнов	Александр	director	20000.00	+380564765145	semenovaleksandr@gmail.com	semenov_alksndr	1
...
17	Егоров	Арнольд	manager	15000.00	+380978872111	ArnoldEgorov@gmail.com	arnoldEgorov	3
21	Островская	Лилиана	manager	15000.00	+380977681543	ostrovskaja_lili@gmail.com	lilianaOst	4
...
18	Рыбакова	Мария	admin	13500.00	+380976761559	ribakovaMarija@gmail.com	ribakova_marija	3
...
15	Павлов	Сергей	shop_assistant	10000.00	+380777886145	PavlovSegei@gmail.com	PavlovSergei	2
19	Кутузова	Анастасия	shop_assistant	10000.00	+380978343145	kutuzova1922@gmail.com	anastasijaKutuzova	3
23	Кольцов 	Алексей	shop_assistant	10000.00	+380971123456	ColcevAlik@gmail.com	colcevaleksei	4
...
205	dummy_client	client	146512fc46367d414ad21a0e1efc8bbd798a975f12b8384510f2d1ad5d8c299c
\.
...
SELECT pg_catalog.setval('public.author_author_id_seq', 33, true);
...
SELECT pg_catalog.setval('public.authority_authority_id_seq', 27, true);
...
SELECT pg_catalog.setval('public.book_book_id_seq', 55, true);
...
SELECT pg_catalog.setval('public.chosen_chosen_id_seq', 140, true);
...
SELECT pg_catalog.setval('public.client_order_order_id_seq', 146, true);
...
SELECT pg_catalog.setval('public.client_review_review_id_seq', 77, true);
...
SELECT pg_catalog.setval('public.edition_edition_number_seq', 25, true);
...
SELECT pg_catalog.setval('public.employee_employee_id_seq', 45, true);
...
--
-- Name: FUNCTION add_review(user_login character varying, book_title character varying, book_publ_date date, user_review_text text); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.add_review(user_login character varying, book_title character varying, book_publ_date date, user_review_text text) TO user_admin;
...
GRANT ALL ON FUNCTION public.available_books_main_info(author_name character varying, book_title character varying, book_genre character varying, min_price numeric, max_price numeric) TO user_checker;
...
--
-- Name: FUNCTION delete_shop_review(shop_id_ integer, user_login_ character varying, review_date_ timestamp without time zone, review_text_ text); Type: ACL; Schema: public; Owner: postgres
--

REVOKE ALL ON FUNCTION public.delete_shop_review(shop_id_ integer, user_login_ character varying, review_date_ timestamp without time zone, review_text_ text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.delete_shop_review(shop_id_ integer, user_login_ character varying, review_date_ timestamp without time zone, review_text_ text) TO user_client;
GRANT ALL ON FUNCTION public.delete_shop_review(shop_id_ integer, user_login_ character varying, review_date_ timestamp without time zone, review_text_ text) TO user_admin;
...
--
-- Name: FUNCTION delete_user(login character varying); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.delete_user(login character varying) TO user_admin;


--
-- Name: FUNCTION delete_user_review_about_book(review_date_ timestamp without time zone, user_login_ character varying, review_text_ text); Type: ACL; Schema: public; Owner: postgres
--

REVOKE ALL ON FUNCTION public.delete_user_review_about_book(review_date_ timestamp without time zone, user_login_ character varying, review_text_ text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.delete_user_review_about_book(review_date_ timestamp without time zone, user_login_ character varying, review_text_ text) TO user_checker;
GRANT ALL ON FUNCTION public.delete_user_review_about_book(review_date_ timestamp without time zone, user_login_ character varying, review_text_ text) TO user_client;
GRANT ALL ON FUNCTION public.delete_user_review_about_book(review_date_ timestamp without time zone, user_login_ character varying, review_text_ text) TO user_admin;
...
GRANT ALL ON FUNCTION public.get_client_info(client_login_ character varying) TO user_checker;
...
GRANT ALL ON FUNCTION public.get_min_max_book_prices() TO user_checker;
...
--
-- Name: FUNCTION get_reviews_about_book(book_title text); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.get_reviews_about_book(book_title text) TO user_admin;
...
GRANT ALL ON FUNCTION public.insert_user_book_review(user_login character varying, book_title character varying, book_publishing_date date, user_review_text text) TO user_checker;
...
--
-- Name: FUNCTION make_discount(); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.make_discount() TO user_admin;
...
GRANT ALL ON FUNCTION public.reduce_available_books() TO user_checker;
...
--
-- Name: FUNCTION seconds_to_dhms(seconds integer); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.seconds_to_dhms(seconds integer) TO user_admin;
...
GRANT ALL ON FUNCTION public.update_client_info(login character varying, subject character varying, new_value character varying) TO user_checker;
